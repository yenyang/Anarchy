    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public LocaleEN(AnarchyModSettings setting)
        {
            ...
            m_Localization = new Dictionary<string, string>()
            {
                ...
                { TooltipTitleKey("ElevationLock"),         "Transform Lock" },
                { TooltipDescriptionKey("ElevationLock"),  "Prevents game systems from changing position (including elevation) and rotation. You can still change position and rotation with mods." },
                ...
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ToggleAnarchyActionName), "Anarchy Toggle Keybind" },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ResetElevationActionName), "Reset Elevation Keybind" },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ElevationStepActionName), "Elevation Step Keybind" },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ElevationActionName, Game.Input.AxisComponent.Positive), "Increase Elevation Keybind" },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ElevationActionName, Game.Input.AxisComponent.Negative), "Decrease Elevation Keybind" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(AnarchyModSettings.NetworkUpgradesPrefabs)), "Adds assets for Retaining Wall, Quay, Elevated, and Tunnel as road upgrades in the vanilla asset menu. These were originally part of Extended Road Upgrades by ST-Apps and have been incorporated into this mod with permission." },
                { m_Setting.GetOptionLabelLocaleID(nameof(AnarchyModSettings.ReplaceUpgradesBehavior)), "Replace Upgrades By Default" },
                { m_Setting.GetOptionDescLocaleID(nameof(AnarchyModSettings.ReplaceUpgradesBehavior)), "With this option enabled and while replacing networks, upgrades will by default be removed/added per tool options toggles. If disabled, they will be preserved if possible by default. You can change the behavior in-game by selecting the individual replace toggles for each row of options." },
                ...
                { TooltipDescriptionKey("AnarchyComponentsTool"), "Add or remove transform lock or anarchy components using a tool with radius or single selection. Radius is recommended for Anarchy component since you can see overridden objects and interact with them. Single selection cannot select overriden objects and they are not visible." },
                ...
                { "Assets.DESCRIPTION[RetainingWall01]", "Network lowered below terrain with one or two walls along one or both sides. Can be applied to left or right side of applicable networks." },
                ...
                { SectionLabel("ElevationVariance"), "Elevation Variance" },
                { TooltipDescriptionKey("IncreaseElevationVariance"), "Increases the range of relative elevations that are randomly assigned. Range is +/- the value shown. Customize keybind in options menu." },
                { TooltipDescriptionKey("DecreaseElevationVariance"), "Decreases the range of relative elevations that are randomly assigned. Range is +/- the value shown. Customize keybind in options menu." },
                { TooltipDescriptionKey("ElevationVarianceStep"),  "Changes the rate in which the elevation variance changes. Customize keybind in options menu." },
                { TooltipDescriptionKey("ResetElevationVariance"), "Resets the Elevation Variance to 0." },
                { TooltipDescriptionKey("ShowElevationVariance"), "Toggles whether to show the Elevation Variance tool option." },
                { m_Setting.GetOptionLabelLocaleID(nameof(AnarchyModSettings.IncreaseElevationVariation)), "Increase Elevation Variance" },
                { m_Setting.GetOptionDescLocaleID(nameof(AnarchyModSettings.IncreaseElevationVariation)), "A keybind to increase the elevation variance range of relative elevations randomly assigned to objects during placement. Range is +/- the value shown." },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ElevationVariationActionName, Game.Input.AxisComponent.Positive), "Increase Elevation Variance Keybind" },
                { m_Setting.GetOptionLabelLocaleID(nameof(AnarchyModSettings.DecreaseElevationVariation)), "Decrease Elevation Variance" },
                { m_Setting.GetOptionDescLocaleID(nameof(AnarchyModSettings.DecreaseElevationVariation)), "A keybind to decrease the elevation variance range of relative elevations randomly assigned to objects during placement. Range is +/- the value shown." },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ElevationVariationActionName, Game.Input.AxisComponent.Negative), "Decrease Elevation Variance Keybind" },
                { m_Setting.GetOptionLabelLocaleID(nameof(AnarchyModSettings.ResetElevationVariation)), "Reset Elevation Variance" },
                { m_Setting.GetOptionDescLocaleID(nameof(AnarchyModSettings.ResetElevationVariation)), "A keybind to reset the elevation variance range to 0." },
                { m_Setting.GetBindingKeyLocaleID(AnarchyModSettings.ElevationVariationResetActionName), "Reset Elevation Variance Keybind" },
