...
namespace Anarchy.ExtendedRoadUpgrades
{
    ...
    internal static class UpgradesManager
    {
        ...
        internal static void Install()
        {
            ...
            if (installed)
            {
                AnarchyMod.Instance.Log.Info($"{logHeader} Extended Upgrades is installed, skipping");
                return;
            }

            AnarchyMod.Instance.Log.Info($"{logHeader} Installing Extended Upgrades");
            ...
            AnarchyMod.Instance.Log.Debug($"{logHeader} Retrieved World instance.");

            // Getting PrefabSystem instance from World
            prefabSystem = world.GetExistingSystemManaged<PrefabSystem>();
            if (prefabSystem == null)
            {
                AnarchyMod.Instance.Log.Error($"{logHeader} Failed retrieving PrefabSystem instance, exiting.");
                return;
            }

            AnarchyMod.Instance.Log.Debug($"{logHeader} Retrieved PrefabSystem instance.");
            ...
            AnarchyMod.Instance.Log.Debug($"{logHeader} Retrieved Prefabs list.");

            // Getting the original Grass Prefab
            var grassUpgradePrefab = prefabs.FirstOrDefault(p => p.name == "Grass");
            if (grassUpgradePrefab == null)
            {
                AnarchyMod.Instance.Log.Error($"{logHeader} Failed retrieving the original Grass Prefab instance, exiting.");
                return;
            }

            AnarchyMod.Instance.Log.Debug($"{logHeader} Retrieved the original Grass Prefab instance.");
            ...
            {
                AnarchyMod.Instance.Log.Error($"{logHeader} Failed retrieving the original Grass Prefab's UIObject instance, exiting.");
                return;
            }

            AnarchyMod.Instance.Log.Debug($"{logHeader} Retrieved the original Grass Prefab's UIObject instance.");
            ...
            foreach (var upgradeMode in ExtendedRoadUpgrades.Modes)
            {
                ...
                {
                    AnarchyMod.Instance.Log.Debug($"{logHeader} [{upgradeMode.ObsoleteId}] Already exists.");
                    return;
                }
                ...
                AnarchyMod.Instance.Log.Info($"{logHeader} [{upgradeMode.Id}] Successfully created and added our cloned Prefab to PrefabSystem.");
                ...
                AnarchyMod.Instance.Log.Info($"{logHeader} Ready to listen to GameManager loading events.");

                // Mark the Install as already executed
                installed = true;

                AnarchyMod.Instance.Log.Info($"{logHeader} Completed.");
            }
        }
        ...
        private static void GameManager_onGameLoadingComplete(Colossal.Serialization.Entities.Purpose purpose, Game.GameMode mode)
        {
            ...
            if (postInstalled)
            {
                AnarchyMod.Instance.Log.Info($"{logHeader} Already executed before, skipping.");
                return;
            }
            ...
            {
                AnarchyMod.Instance.Log.Info($"{logHeader} Game mode is {mode}, skipping.");
                return;
            }

            AnarchyMod.Instance.Log.Info($"{logHeader} Started.");
            ...
            if (grassUpgradePrefabData.Equals(default(PlaceableNetData)))
            {
                ...
                AnarchyMod.Instance.Log.Error($"{logHeader} Failed retrieving the original Grass Prefab's PlaceableNetData instance, exiting.");
                return;
            }

            AnarchyMod.Instance.Log.Debug($"{logHeader} Retrieved the original Grass Prefab's PlaceableNetData instance.");
            ...
            foreach (var upgradeMode in ExtendedRoadUpgrades.Modes)
            {
                ...
                {
                    AnarchyMod.Instance.Log.Error($"{logHeader} [{upgradeMode.Id}] Failed retrieving the cloned Grass Prefab instance, exiting.");
                    return;
                }

                AnarchyMod.Instance.Log.Debug($"{logHeader} [{upgradeMode.Id}] Retrieved the cloned Grass Prefab instance.");
                ...
                {
                    // This type is not nullabe so we check equality with the default empty data
                    AnarchyMod.Instance.Log.Error($"{logHeader} [{upgradeMode.Id}] Failed retrieving the cloned Grass Prefab's PlaceableNetData instance, exiting.");
                    return;
                }

                AnarchyMod.Instance.Log.Debug($"{logHeader} [{upgradeMode.Id}] Retrieved the cloned Grass Prefab's PlaceableNetData instance.");
                ...
                AnarchyMod.Instance.Log.Info($"{logHeader} [{upgradeMode.Id}] Successfully set flags for our cloned Prefab to {clonedGrassUpgradePrefabData.m_SetUpgradeFlags.ToJSONString()} and {clonedGrassUpgradePrefabData.m_UnsetUpgradeFlags.ToJSONString()}.");
            }

            // Mark the Prefix as already executed
            postInstalled = true;

            AnarchyMod.Instance.Log.Info($"{logHeader} Extended Road Upgrades Completed.");
        }
    }
}
