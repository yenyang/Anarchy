        ...
        protected override void OnUpdate()
        {
            ...
            foreach (Entity currentEntity in entities)
            {
                if (EntityManager.TryGetComponent(currentEntity, out NetCompositionData netCompositionData))
                {
                    ...
                    {
                        netCompositionData.m_HeightRange.min = Mathf.Clamp(-1f * Mod.Instance.Settings.MinimumClearanceBelowElevatedNetworks, netCompositionData.m_HeightRange.min, netCompositionData.m_HeightRange.max);
